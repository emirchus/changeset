...
void main() {
  ...
  group('Config and workspace', () {
    test('resolveConfig finds changeset.yaml and returns root and packages',
        () async {
...
''');
    ...
    test('add -W creates changeset in root .changesets with package: line',
        () async {
...
packages:
  foo: packages/foo
''');
        await Directory('${temp.path}${sep}packages${sep}foo').create(recursive: true);
        await File('${temp.path}${sep}packages${sep}foo${sep}pubspec.yaml')
            .writeAsString('name: foo\nversion: 1.0.0\n');
        final binPath = '${Directory.current.path}$sep''bin$sep''changeset.dart';

        final result = await Process.run(
          'dart',
          ['run', binPath, 'add', '-W', 'foo', '--type', 'patch', '--summary', 'test change'],
          runInShell: false,
...
''');
        ...
        await File('${temp.path}${sep}packages${sep}foo${sep}pubspec.yaml')
            .writeAsString('name: foo\nversion: 1.0.0\n');
        final csFile = File('${temp.path}${sep}.changesets${sep}2025-01-01-test.md');
...
''');
        ...
        final pubspecContent =
            await File('${temp.path}${sep}packages${sep}foo${sep}pubspec.yaml').readAsString();
        ...
        final archivedDir =
            Directory('${temp.path}${sep}.changesets${sep}archived${sep}foo${sep}1.0.1');
